...
const PaywallPage = ({ clientSecret }) => {
      ...
      expressCheckoutElement.on("confirm", async (event) => {
        try {
          ...
          if (paymentIntent && paymentIntent.status === "succeeded") {
            console.log(
              "Subscription payment successful with Apple Pay/Google Pay!",
            );
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const { client_secret, subscriptionId } = await res.json();
      console.log(
        "Received client_secret from /api/checkout_sessions:",
        client_secret,
      );
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        >
          {t(
            "Access personalized recipes, manage your pantry, and receive tailored meal plans with our premium Nutrition features.",
          )}
        </Typography>
          ...
          <Grid item xs={12} sm={4}>
            <Card variant="outlined">
              <CardContent>
                ...
                >
                  {t(
                    "Get AI-generated recipes tailored to your available ingredients and dietary preferences.",
                  )}
                </Typography>
              </CardContent>
            </Card>
          ...
          <Grid item xs={12} sm={4}>
            <Card variant="outlined">
              <CardContent>
                ...
                >
                  {t(
                    "Easily keep track of your ingredients and their quantities, and never miss out on what you need.",
                  )}
                </Typography>
              </CardContent>
            </Card>
          ...
          <Grid item xs={12} sm={4}>
            <Card variant="outlined">
              <CardContent>
                ...
                >
                  {t(
                    "Generate meal plans and grocery lists quickly based on what's available in your pantry.",
                  )}
                </Typography>
              </CardContent>
            </Card>
        ...
        <Box mt={4} p={2} textAlign="center">
          ...
          <Grid container spacing={2} justifyContent="center">
            ...
            <Grid item xs={6}>
              <Typography variant="body1" fontWeight="bold">
                {t("Premium Users - $5/month")}
              </Typography>{" "}
              {/* Pricing in the header */}
              <Typography variant="body2" color="textSecondary">
                {t("All free features")}
              </Typography>
              <Typography variant="body2" color="textSecondary">
...
const PaywallPageWithStripe = () => {
  ...
  if (!clientSecret) {
    ...
    ); // Show a loading spinner while clientSecret is being fetched
  }
